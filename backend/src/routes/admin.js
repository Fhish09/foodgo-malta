...
// Auth gate — explicit middleware functions (avoids path-to-regexp issues)
...
router.patch('/restaurants/:id', function (req, res) {
  try {
    ...
    db.prepare('UPDATE restaurants SET name = COALESCE(?, name), category = COALESCE(?, category), description = COALESCE(?, description), town = COALESCE(?, town), delivery_fee = COALESCE(?, delivery_fee), is_active = COALESCE(?, is_active) WHERE id = ?').run(name != null ? name : null, category != null ? category : null, description != null ? description : null, town != null ? town : null, delivery_fee != null ? Number(delivery_fee) : null, is_active != null ? (is_active ? 1 : 0) : null, req.params.id);
...
router.get('/promos', function (_req, res) {
  try {
...
router.post('/promos', function (req, res) {
  try {
    const { code, title, description, discount_type, discount_value, min_order, active } = req.body || {};
    if (!code || !discount_type || discount_value == null) return res.status(400).json({ error: 'code, discount_type, discount_value required' });
    const id = uuid();
    db.prepare('INSERT INTO promos (id, code, title, description, discount_type, discount_value, min_order, active) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(id, String(code).trim().toUpperCase(), title || code, description || '', discount_type, Number(discount_value), min_order != null ? Number(min_order) : 0, active === 0 || active === false ? 0 : 1);
...
router.patch('/promos/:id', function (req, res) {
  try {
    ...
    const { code, title, description, discount_type, discount_value, min_order, active } = req.body || {};
    db.prepare('UPDATE promos SET code = COALESCE(?, code), title = COALESCE(?, title), description = COALESCE(?, description), discount_type = COALESCE(?, discount_type), discount_value = COALESCE(?, discount_value), min_order = COALESCE(?, min_order), active = COALESCE(?, active) WHERE id = ?').run(code != null ? String(code).trim().toUpperCase() : null, title != null ? String(title) : null, description != null ? String(description) : null, discount_type != null ? discount_type : null, discount_value != null ? Number(discount_value) : null, min_order != null ? Number(min_order) : null, active != null ? (active ? 1 : 0) : null, req.params.id);
...
router.delete('/promos/:id', function (req, res) {
  try {
...
router.get('/orders', function (_req, res) {
  try {
...
router.patch('/orders/:id/status', function (req, res) {
  try {
...
router.get('/drivers', function (_req, res) {
  try {
...
router.patch('/drivers/:id', function (req, res) {
  try {
    ...
    db.prepare('UPDATE drivers SET name = COALESCE(?, name), phone = COALESCE(?, phone), vehicle = COALESCE(?, vehicle), status = COALESCE(?, status), active = COALESCE(?, active) WHERE id = ?').run(name != null ? String(name).trim() : null, phone != null ? String(phone).trim() : null, vehicle != null ? String(vehicle).trim() : null, status != null ? status : null, active != null ? (active ? 1 : 0) : null, req.params.id);
...
router.post('/orders/:id/assign', function (req, res) {
  try {
